...
S2E_DEFINE_PLUGIN(TraceMonitor, "Kill dead loops of executed translation blocks", "TraceMonitor");

namespace {

class TraceMonitorState : public PluginState {
private:
...
public:
    virtual TraceMonitorState *clone() const {
        return new TraceMonitorState(*this);
    }
    ...
    static PluginState *factory(Plugin *p, S2EExecutionState *s) {
        return new TraceMonitorState();
    }

    virtual ~TraceMonitorState() {
    }
...
void TraceMonitor::initialize() {

    s2e()->getCorePlugin()->onTranslateBlockEnd.connect(
        sigc::mem_fun(*this, &TraceMonitor::onTranslateBlockEnd));
    ...
    blockStartConnection = s2e()->getCorePlugin()->onTranslateBlockStart.connect(
        sigc::mem_fun(*this, &TraceMonitor::onTranslateBlockStart));
    // use for invaild pc
    invalidPCAccessConnection = s2e()->getCorePlugin()->onInvalidPCAccess.connect(
        sigc::mem_fun(*this, &TraceMonitor::onInvalidPCAccess));
    onARMFunctionConnection = s2e()->getPlugin<ARMFunctionMonitor>();
            onARMFunctionConnection->onARMFunctionCallEvent.connect(
                sigc::mem_fun(*this, &TraceMonitor::onARMFunctionCall));
            
            
            onARMFunctionConnection->onARMFunctionReturnEvent.connect(
                sigc::mem_fun(*this, &TraceMonitor::onARMFunctionReturn));
}
void TraceMonitor::onARMFunctionCall(S2EExecutionState *state, uint32_t caller_pc, uint64_t function_hash) {
    DECLARE_PLUGINSTATE(TraceMonitorState, state);
...
void TraceMonitor::onARMFunctionReturn(S2EExecutionState *state, uint32_t return_pc) {
    DECLARE_PLUGINSTATE(TraceMonitorState, state);
    if (state->regs()->getInterruptFlag()) {
...
void TraceMonitor::onTranslateBlockEnd(ExecutionSignal *signal, S2EExecutionState *state,
                                                 TranslationBlock *tb, uint64_t pc, bool staticTarget,
                                                 uint64_t staticTargetPc) {
    signal->connect(
        sigc::bind(sigc::mem_fun(*this, &TraceMonitor::onInvalidLoopDetection), (unsigned) tb->se_tb_type));
}

void TraceMonitor::onTranslateBlockStart(ExecutionSignal *signal, S2EExecutionState *state,
                                                   TranslationBlock *tb, uint64_t pc) {
    signal->connect(sigc::mem_fun(*this, &TraceMonitor::onKillandAlivePoints));
}
...
void TraceMonitor::onCacheModeMonitor(S2EExecutionState *state, uint64_t pc) {
    DECLARE_PLUGINSTATE(TraceMonitorState, state);

    getDebugStream() << "TraceMonitor in cache mode " << plgState->getnewtbnum() << " pc = " << hexval(pc)
...
void TraceMonitor::onInvalidStatesKill(S2EExecutionState *state, uint64_t pc, InvalidStatesType type,
...
bool TraceMonitor::onModeSwitchandTermination(S2EExecutionState *state, uint64_t pc) {
    DECLARE_PLUGINSTATE(TraceMonitorState, state);
...
void TraceMonitor::onKillandAlivePoints(S2EExecutionState *state, uint64_t pc) {
    DECLARE_PLUGINSTATE(TraceMonitorState, state);
...
void TraceMonitor::onInvalidPCAccess(S2EExecutionState *state, uint64_t addr) {
   // DECLARE_PLUGINSTATE(TraceMonitorState, state);
    if (!init_cache_mode) {
...
void TraceMonitor::onInvalidLoopDetection(S2EExecutionState *state, uint64_t pc, unsigned source_type) {
    DECLARE_PLUGINSTATE(TraceMonitorState, state);
//450-478
    // cache mode
    if (g_s2e_cache_mode) {
    ...
    if (state->regs()->getInterruptFlag() && state->regs()->getExceptionIndex() > 15) {
        ...
        if (plgState->inctbnum(pc)) {
            getInfoStream() << "TraceMonitor in learning mode new tb num = " << plgState->getnewtbnum()
                                << " pc = " << hexval(pc) << "\n";
        }
    }
...
/*void TraceMonitor::recordTBMap(S2EExecutionState *state) {*/
// DECLARE_PLUGINSTATE(TraceMonitorState, state);
// std::string fileName;
